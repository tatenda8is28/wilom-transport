  ...
  return (
    <div className="bg-white rounded-[32px] overflow-hidden border border-slate-100 shadow-sm hover:shadow-2xl transition-all duration-500 group">
      <div className="relative aspect-[16/11] overflow-hidden text-left">
        <img src={vehicle.main_image} alt="" className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-700" />
        <div className="absolute top-5 left-5">
           <span className="bg-[#22c55e] text-white text-[10px] font-black px-4 py-2 rounded-full uppercase tracking-widest shadow-lg">{vehicle.badge}</span>
        </div>
        <div className="absolute bottom-5 left-5 bg-white/90 backdrop-blur px-3 py-1.5 rounded-xl text-[10px] font-black uppercase">{vehicle.body_type}</div>
        <div className="absolute bottom-5 right-5 bg-[#0f172a] text-white px-3 py-1.5 rounded-xl text-[10px] font-black uppercase">{vehicle.make}</div>
      </div>
      <div className="p-8 text-left">
        ...
        <div className="flex justify-between items-end border-t border-slate-100 pt-6">
          <div>
            ...
            <span className="text-2xl font-black text-[#0f172a]">£{Number(vehicle.uk_price_gbp).toLocaleString()}</span>
          </div>
    ...
    <div className="bg-slate-50 min-h-screen">
      <section className="relative bg-[#0f172a] h-[85vh] flex items-center overflow-hidden pb-32">
        ...
        <div className="max-w-7xl mx-auto px-4 relative z-10 w-full text-left">
           ...
           <div className="flex flex-wrap gap-5">
              <button className="bg-[#dc2626] text-white px-12 py-5 rounded-xl font-black text-lg flex items-center gap-3 group shadow-2xl">Browse Vehicles <ArrowRight size={22} className="group-hover:translate-x-1 transition-transform"/></button>
              <button className="bg-white/5 backdrop-blur-md border border-white/20 text-white px-12 py-5 rounded-xl font-black text-lg hover:bg-white/10 transition flex items-center gap-3"><Phone size={22} className="text-[#dc2626]"/> Contact Us</button>
           </div>
        </div>
      </section>

      {/* Hero Search placeholder logic here... */}
      ...
      <section className="max-w-7xl mx-auto px-4 py-32">
        <div className="flex flex-col md:flex-row justify-between items-end gap-6 mb-20 text-left">
           ...
           <button className="bg-white border-2 border-slate-200 px-10 py-4 rounded-2xl font-black text-sm hover:bg-slate-900 hover:text-white transition-all flex items-center gap-3 group shadow-sm">View All Inventory <span className="group-hover:translate-x-1 transition-transform text-[#dc2626]">→</span></button>
        </div>
        ...
        {loading ? (
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-12">
            {featured.map(v => <VehicleCard key={vehicle.id} vehicle={v} />)}
          </div>
        )}
      </section>
    </div>
