    ...
    <div className="max-w-6xl mx-auto px-4 -mt-12 md:-mt-16 relative z-30">
      <div className="bg-white p-4 md:p-6 rounded-2xl md:rounded-[32px] shadow-2xl border border-slate-100 grid grid-cols-1 md:grid-cols-4 gap-4 items-center">
        <div className="relative group">
          <label className="hidden md:block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2 px-4 text-left">Brand</label>
        ...
        <div className="relative group">
          <label className="hidden md:block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2 px-4 text-left">Vehicle Type</label>
          <div className="flex items-center justify-between bg-slate-50 px-4 py-3 rounded-xl cursor-pointer hover:bg-slate-100 transition">
        ...
        <div className="relative group">
          <label className="hidden md:block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2 px-4 text-left">Price Range</label>
          <div className="flex items-center justify-between bg-slate-50 px-4 py-3 rounded-xl cursor-pointer hover:bg-slate-100 transition">
...
function VehicleCard({ vehicle }) {
  return (
    <div className="bg-white rounded-[32px] overflow-hidden border border-slate-100 shadow-sm hover:shadow-2xl transition-all duration-500 group">
      ...
      <div className="p-8">
        <h3 className="font-black text-xl md:text-2xl text-slate-800 leading-tight mb-5 group-hover:text-[#dc2626] transition-colors text-left">
          {vehicle.name}
        </h3>
        
        <div className="flex items-center gap-4 text-slate-400 font-bold text-[10px] md:text-[12px] mb-8 uppercase tracking-tighter">
        ...
        <div className="flex justify-between items-end border-t border-slate-100 pt-6">
          <div className="text-left">
            ...
            <span className="text-xl md:text-2xl font-black text-slate-900 tracking-tight">${vehicle.ukPrice.toLocaleString()}</span>
          </div>
          <div className="text-right">
            <span className="text-[10px] font-black text-[#dc2626] uppercase tracking-widest block mb-1">Delivered ZIM</span>
            <span className="text-xl md:text-2xl font-black text-[#dc2626] tracking-tight">${vehicle.zimPrice.toLocaleString()}</span>
          </div>
        </div>
      </div>
    </div>
...
export default function Home() {
  ...
  return (
    <div className="bg-slate-50 min-h-screen">
      {/* HERO SECTION - Added extra padding bottom to fix overlay overlap */}
      <section className="relative bg-[#0f172a] min-h-[80vh] md:h-auto md:min-h-[85vh] flex items-center overflow-hidden pt-12 pb-32 md:pb-48">
        <div className="absolute inset-0 z-0">
           <img src="https://images.unsplash.com/photo-1580674285054-bed31e145f59?auto=format&fit=crop&q=80" className="w-full h-full object-cover opacity-20" alt="Background" />
        ...
        <div className="max-w-7xl mx-auto px-4 relative z-10 w-full text-left">
           <div className="bg-[#dc2626]/10 border border-[#dc2626]/30 px-5 py-2 rounded-full text-[#dc2626] text-[10px] md:text-[11px] font-black inline-flex items-center gap-3 mb-10 uppercase tracking-widest">
           ...
           <div className="flex flex-wrap gap-5">
              <button className="bg-[#dc2626] text-white px-12 py-5 rounded-xl font-black text-lg hover:bg-red-700 transition shadow-2xl flex items-center gap-3 group">
      ...
      <section className="max-w-7xl mx-auto px-4 py-20 md:py-32">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6 mb-20">
           <div className="text-left">
              ...
              <h2 className="text-4xl md:text-6xl font-black tracking-tighter text-[#0f172a] leading-none">Latest Arrivals</h2>
              <p className="text-slate-500 mt-4 text-lg md:text-xl font-bold max-w-lg">Hand-picked heavy vehicles ready for delivery.</p>
           </div>
           <button className="bg-white border-2 border-slate-200 px-10 py-4 rounded-2xl font-black text-sm hover:bg-slate-900 hover:text-white transition-all flex items-center gap-3 group">
