              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                {vehicles.length === 0 ? (
                    <div className="col-span-full py-32 text-center bg-white rounded-[4rem] border border-dashed border-slate-200">
                        ...
                        <p className="text-slate-400 font-black uppercase tracking-widest text-sm">No vehicles match your search.</p>
                    </div>
                ) : (
                  vehicles.map(v => (
                    <div key={v.id} className="bg-white rounded-[3.5rem] overflow-hidden border border-slate-100 shadow-sm hover:shadow-2xl transition-all duration-700 group flex flex-col h-full">
                      ...
                      <div className="p-10 text-left flex-grow flex flex-col">
                        <div className="flex justify-between items-start mb-6">
                          <div>
                            <h3 className="font-black text-4xl text-[#0f172a] tracking-tighter leading-none mb-2 uppercase italic">{v.make}</h3>
                            <p className="text-slate-400 font-bold text-xl uppercase">{v.model}</p>
                          </div>
                        ...
                        {/* PRICING TABLE */}
                        <div className="grid grid-cols-2 gap-4 bg-slate-50 p-8 rounded-[2.5rem] mb-8 border border-slate-100">
                          <div>
                        ...
                        {/* VIEW SPECS ACTION - Primary focus now */}
                        <Link 
                          to={`/vehicle/${v.id}`}
                          className="w-full bg-[#0f172a] text-white py-6 rounded-[2rem] font-black text-lg flex items-center justify-center gap-3 hover:bg-slate-800 transition-all shadow-xl active:scale-95"
                        >
                          <Eye size={24} />
                          View Full Details
                        </Link>
